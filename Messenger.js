...
module.exports = Messenger = async (client, message) => {
    try {
        ...
        const mess = {
            wait: '[ WAIT ] Tá carregando ⏳ espera ai que tu né a xuxa não',
            error: {
                St: '[❗] Envia a imagem com a legenda *!sticker* ou a tag de imagem que foi enviada',
                Qm: '[❗] Não vai dar não, talvez o tema não esteja disponível! ⛔',
                Yt3: '[❗] Não vai dar não, não foi possível converter para mp3! ⛔',
                Yt4: '[❗] Não vai dar não, talvez o erro foi causado pelo sistema. ⛔',
                Ig: '[❗] Não vai dar não, talvez porque a conta é privada ⛔',
                Ki: '[❗] Mano você é comum, esse comando é apenas pra administrador 😕',
                Ad: '[❗] Não foi possível adicionar alvo, talvez porque é privado ⛔',
                Iv: '[❗] Esse link ai não presta não 😕'
            }
        }
        ...
        case '!readme':
            ...
            break
        case '!about':
            client.sendLinkWithAutoPreview(from, 'ExTRUPICIO 🦍', info)
            break
       ...
       case '!lihatblock':
        ...
        case '!stikergif':
            ...
            if (isMedia || isQuotedVideo) {
                if (mimetype === 'video/mp4' && message.duration < 10 || mimetype === 'image/gif' && message.duration < 10) {
                    ...
                    client.reply(from, '[WAIT] Tá carregando ⏳ espera ai que tu né a xuxa não!', id)
                    ...
                    await exec(`gify ${filename} ./media/stickergf.gif --fps=30 --scale=240:240`, async function (error, stdout, stderr) {
                        ...
                        .catch(() => {
                            client.reply(from, 'Vai dar não, arquivo é grande 🙅🏻‍♂️', id)
                        })
                    })
                  } else {
                    client.reply(from, `[❗] Envie um GIF com uma legenda *${prefix}stickergif* max 10 sec!`, id)
                   }
                } else {
        ...
        case '!stickergiphy':
            if (!isMe) return
            if (args.length !== 1) return client.reply(from, `Escreveu errado analfabeto 😒.\Tem que escrever com ${prefix}stickergiphy <link_giphy>`, id)
            const isGiphy = url.match(new RegExp(/https?:\/\/(www\.)?giphy.com/, 'gi'))
            const isMediaGiphy = url.match(new RegExp(/https?:\/\/media.giphy.com\/media/, 'gi'))
            if (isGiphy) {
                const getGiphyCode = url.match(new RegExp(/(\/|\-)(?:.(?!(\/|\-)))+$/, 'gi'))
                if (!getGiphyCode) { return client.reply(from, 'Deu ruim ao recuperar o código giphy 😕', id) }
                const giphyCode = getGiphyCode[0].replace(/[-\/]/gi, '')
                const smallGifUrl = 'https://media.giphy.com/media/' + giphyCode + '/giphy-downsized.gif'
                client.sendGiphyAsSticker(from, smallGifUrl).then(() => {
                    client.reply(from, 'Toma tua figurinha 😎')
                    console.log(`Transformando em figurinha ${processTime(t, moment())} calma ai`)
                }).catch((err) => console.log(err))
            } else if (isMediaGiphy) {
                const gifUrl = url.match(new RegExp(/(giphy|source).(gif|mp4)/, 'gi'))
                if (!gifUrl) { return client.reply(from, 'Deu ruim ao recuperar o código giphy 🙁', id) }
                const smallGifUrl = url.replace(gifUrl[0], 'giphy-downsized.gif')
                client.sendGiphyAsSticker(from, smallGifUrl)
                .then(() => {
                    client.reply(Toma tua figurinha 😎')
                    console.log(`Transformando em figurinha ${processTime(t, moment())} calma ai`)
                })
                .catch(() => {
                    client.reply(from, `Deu certo não`, id)
                })
            } else {
                await client.reply(from, 'Vacilo, a gente aki só usa o link do giphy. 🙁 [Giphy Only]', id)
            }
            break
                 break   
        case '!linkgroup':
        case '!linkgrup':
            if (!isBotGroupAdmins) return client.reply(from, 'Mano tu é membro comum, só ADM faz essas paradas 😕', id)
            if (isGroupMsg) {
                const inviteLink = await client.getGroupInviteLink(groupId);
                client.sendLinkWithAutoPreview(from, inviteLink, `\n *Deixa eu te perguntar uma parada 🤔* *${name}*`)
            } else {
                client.reply(from, '*Foi mal aí, mais não vai dar não 😕:(*', id)
            }
            break
         case '!BAN':
            ...
            if (mentionedJidList.length === 0) return client.reply(from, 'cara menggunakan, kirim perintah lalu *!BAN* @tagmember', id)
      ...
      case '!kpop':
        if (!isGroupMsg) return client.reply(from, 'Onichan Gomenasai harus di group desu!', id)
            ...
            break           
     case '!pokemon':
     if (!isGroupMsg) return client.reply(from, 'Mano tu é esse pokemon ai na moral kkkkkk', id)
